...
const search_songs = async function(req, res) {
  ...
  connection.query(
    `SELECT *
    FROM Songs
    WHERE title LIKE ? AND duration >= ? AND duration <= ? AND plays >= ? AND plays <= ? AND danceability >= ? AND danceability <= ? AND energy >= ? AND energy <= ? AND valence >= ? AND valence <= ? AND explicit <= ?
    ORDER BY title ASC`, [`%${title}%`, durationLow, durationHigh, playsLow, playsHigh, danceabilityLow, danceabilityHigh, energyLow, energyHigh, valenceLow, valenceHigh, explicit],
    (err, data) => {
      if (err || data.length === 0) {
        console.log(err);
        res.json([]);
